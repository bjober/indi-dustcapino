...
private:
    ...
    int lastPulse = -2;
    int lastMoving = -1;
    time_t lastStatusPacket = 0;
    time_t lastStatusRequest = 0;
...
bool readLine(char *buffer, int &nbytes);
void dispatchPacket(char *buffer);

// Packet parsers
void parseHello(const char *buffer);
void parseStatus(const char *buffer);
void parseDHT(const char *buffer);
void parseDiagnostics(char *buffer);
void parseErrors(const char *buffer);
};