...
void loop()
{
  ...
  // --- PWM watchdog ---
  if (now - lastPWMCheck > 3000)
  {
    if (!dustCapServo.attached())
    {
      Serial.println("SERVO_WATCHDOG_REATTACH");
  ...
  if (moving && now - lastServoUpdate > SERVO_INTERVAL)
  {
    ...
    if (fabs(currentAngle - lastStatusAngle) < 0.5)
    {
      if (millis() - servoStallTimer > 3500)
      {
  ...
  if (millis() - lastHostContact > 60000)
  {
    ...
    // close cap if host disappeared
    if (capState != CLOSED)
    {
      Serial.println("FAILSAFE_CLOSE");

      startMove(270.0);   // close cap
    }
  }  // -------- Non-blocking serial --------
